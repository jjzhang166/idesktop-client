...
PackageWidget::PackageWidget(QWidget *parent): QScrollArea(parent)
{
    ...
    connect(buttonGroup,SIGNAL(buttonClicked(int)),tableView,SLOT(categoryChange(int)));
    ...
    connect(tableView, SIGNAL(detail(const QString &)), this, SIGNAL(detail(const QString &)));
    allResize();
}

void PackageWidget::refresh()
{
    tableView->refresh();
}
...
{
    all = new QPushButton("所有软件");
	professional = new QPushButton("专业软件");
	network = new QPushButton("网络应用");
	media = new QPushButton("多媒体");
	driver = new QPushButton("硬件驱动");
	develop = new QPushButton("开发工具");
	other = new QPushButton("其他软件");
