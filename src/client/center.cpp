    ...
    :QWidget(parent)
{
    ...
    setWindowIcon(QIcon(":images/center.png"));
    ...
    connect(pWidget, SIGNAL(detail(const QString &)), this, SLOT(onDetail(const QString &)));
    connect(dWidget, SIGNAL(cancelFromDetail()), this, SLOT(onCancel()));
    connect(dWidget, SIGNAL(doneFromDetail()), this, SLOT(onDone()));
    showMaximized ();
}
...
void AppCenter::onDone()
{
}

void AppCenter::refresh()
{
    pWidget->refresh();
}

void AppCenter::onCancel()
{
    leftBar->setEnabled(true);
    stackedLayout->setCurrentIndex(0);
}

void AppCenter::onDetail(const QString &name)
{
    leftBar->setEnabled(false);
    dWidget->sendName(name, leftBar->currentIndex());
    stackedLayout->setCurrentIndex(1);
}

void AppCenter::resizeEvent(QResizeEvent *event)
{
	QPixmap background(":images/client-bg.png");
