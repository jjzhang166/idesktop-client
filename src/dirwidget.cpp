...
void DirWidget::dragMoveEvent(QDragMoveEvent *event)
{
    ...
    if (QRect(0, _pageSize.height() - 10, _pageSize.width(), 10)\
            .contains(event->pos())) {
    ...
    /* move to the up page */
    else if (QRect(0, 0, _pageSize.width(), 10)\
             .contains(event->pos())) {
        if (_current == 0)
            return;
        dragLeftTimer->start();
...
void DirWidget::dragLeft()
{
    if (!_inDrag)
        return;
    if (!QRect(0, 0, _pageSize.width(), 10)\
            .contains(cursor().pos()))
        return;
    if (_current != 0) {
//        move(_pages[--_current], y());
